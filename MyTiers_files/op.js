...
const opObserverOptions = {
    root: null,
    rootMargin: '50px',
    threshold: 0.1
};

const opImageObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            opImageObserver.unobserve(img);
        }
    });
}, opObserverOptions);

// Modified renderTiersGrid with headers above corresponding columns
function renderTiersGridOp(category, gridElementId) {
    ...
    const existingElements = gridElement.querySelectorAll('.op-tier-header, .op-tier-column');
    if (existingElements.length > 0) {
        console.log('OP grid already rendered, skipping animation');
        return;
    }
    ...
    for (let tierLevel = 1; tierLevel <= 5; tierLevel++) {
        ...
        if (playersInTier.length === 0) {
            ...
            playersInTier.forEach((player, index) => {
                ...
                opImageObserver.observe(cardAvatar);
            });
        }
    ...
    try {
        ...
        console.warn('attachTierInfoButton(op) failed:', e);
    }
...
window.renderTiersGridOp = renderTiersGridOp;

// Main initialization for OP page
async function initOp() {
    ...
    // Load data (no limit for OP page)
...
// if (window.location.pathname === '/op.html' || window.location.pathname.endsWith('/op.html')) {
//     window.addEventListener('load', initOp);
// }